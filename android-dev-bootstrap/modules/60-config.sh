...
_install_management_commands() {
  ...
  local command
  for command in asik-dev ollama camera-ai claude-zai claude-anthropic aider-xai aider-openrouter; do
...
module_config() {
  run_step "Install asik-dev management, camera, and provider commands" _install_management_commands
